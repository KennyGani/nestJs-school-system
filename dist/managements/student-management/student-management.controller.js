"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.StudentManagementController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
let StudentManagementController = class StudentManagementController {
    createStudent() {
        return 'student created';
    }
    updateStudent(studentId) {
        return `This action update student with id = #${studentId}`;
    }
    getStudentWithId(studentId) {
        return `This action get student with id = #${studentId}`;
    }
    getAllStudent() {
        return `This action get all student`;
    }
    deleteStudentWithId(studentId) {
        return `This action delete student with id = #${studentId}`;
    }
};
__decorate([
    (0, common_1.Post)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", String)
], StudentManagementController.prototype, "createStudent", null);
__decorate([
    (0, common_1.Put)('/:studentId'),
    __param(0, (0, common_1.Param)('studentId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", String)
], StudentManagementController.prototype, "updateStudent", null);
__decorate([
    (0, common_1.Get)('/:studentId'),
    __param(0, (0, common_1.Param)('studentId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", String)
], StudentManagementController.prototype, "getStudentWithId", null);
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", String)
], StudentManagementController.prototype, "getAllStudent", null);
__decorate([
    (0, common_1.Delete)('/:studentId'),
    __param(0, (0, common_1.Param)('studentId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", String)
], StudentManagementController.prototype, "deleteStudentWithId", null);
StudentManagementController = __decorate([
    (0, swagger_1.ApiTags)('student-management'),
    (0, common_1.Controller)('student-management')
], StudentManagementController);
exports.StudentManagementController = StudentManagementController;
//# sourceMappingURL=student-management.controller.js.map